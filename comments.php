    ...
    <div class="blog-comments">
        ...
        <div id="show-hide">
            ...
            <?php if (comments_open()) : ?>
                ...
                <?php else : ?>
                    <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" id="comment-form"
                        ...
                        <?php else : ?>
                            <div class="personal">
                                <input type="text" class="name" placeholder="<?php _e('Your Name', 'aletheme'); ?> *"
                                       name="author" id="author" value="<?php echo esc_attr($comment_author); ?>"
                                       tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> required="required">
                                <input type="email" placeholder="<?php _e('Your Email', 'aletheme'); ?> *" class="email"
                                       name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>"
                                       tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> required="required"
                                       email="true">
                                <input type="url" placeholder="<?php _e('Your Site', 'aletheme'); ?>" class="site"
                                       name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>"
                                       tabindex="3">
                            </div>
                        <?php endif; ?>
                        <div class="message-submit">
                            <input name="submit" class="submit" type="submit"
                                   value="<?php _e('SUBMIT', 'aletheme'); ?>"/>
                            <textarea name="comment" placeholder="<?php _e('Say Something', 'aletheme'); ?> *"
                                      id="comment" class="message" required="required"></textarea>
                        </div>
...
<?php elseif (ale_get_option('comments_style') == 'fb') : ?>
    <section class="facebook-comments">
        <div id="fb-comments<?php the_ID() ?>" class="fb-comments" data-href="<?php the_permalink() ?>"
